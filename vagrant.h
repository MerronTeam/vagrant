...
void CreateUser()
{
    ...
    N = n ;
    fclose(stdout) ;
    MessageBox(NULL, "Please restart.", "INFO", MB_ICONINFORMATION) ;
    getch() ;
    exit(0) ;
}
...
void print()
{
    ...
    for (int i = 1 ;i <= MAPSIZE ;i ++)
    {
        for (int j = 1 ;j <= MAPSIZE ;j ++)
        {
            if(OfUser(j, i)&&ADD[i][j] == 1&&City[i][j])
            {
